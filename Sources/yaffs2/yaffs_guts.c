...
u8 *yaffs_get_temp_buffer(struct yaffs_dev * dev)
{
	...
	yaffs_trace(YAFFS_TRACE_BUFFERS, "Out of temp buffers%s", "" );
	/*
...
void yaffs_release_temp_buffer(struct yaffs_dev *dev, u8 *buffer)
{
	...
	if (buffer) {
		...
		yaffs_trace(YAFFS_TRACE_BUFFERS,
			"Releasing unmanaged temp buffer%s", "" );
		kfree(buffer);
...
static int yaffs_find_alloc_block(struct yaffs_dev *dev)
{
	...
	if (dev->n_erased_blocks < 1) {
		...
		yaffs_trace(YAFFS_TRACE_ERROR,
		  "yaffs tragedy: no more erased blocks%s", "" );
			     ...
			     struct yaffs_block_info **block_ptr)
{
	...
	if (dev->n_erased_blocks < dev->param.n_reserved_blocks
	    && dev->alloc_page == 0)
		yaffs_trace(YAFFS_TRACE_ALLOCATE, "Allocating reserve%s", "" );
			    ...
			    int nand_chunk, int in_scan)
{
	...
	if (in->variant_type != YAFFS_OBJECT_TYPE_FILE) {
		...
		if (!in_scan) {
			yaffs_trace(YAFFS_TRACE_ERROR,
				"yaffs tragedy:attempt to put data chunk into a non-file%s"
				, "" );
			BUG();
		}
	...
	if (in_scan != 0) {
		...
		if (existing_cunk > 0) {
			...
			if (existing_cunk <= 0) {
				...
				yaffs_trace(YAFFS_TRACE_ERROR,
					"yaffs tragedy: existing chunk < 0 in scan%s"
					, "" );
...
void yaffs_add_obj_to_dir(struct yaffs_obj *directory, struct yaffs_obj *obj)
{
	if (!directory) {
		yaffs_trace(YAFFS_TRACE_ALWAYS,
			"tragedy: Trying to add an object to a null pointer directory%s"
			, "" );
		BUG();
		return;
	}
	if (directory->variant_type != YAFFS_OBJECT_TYPE_DIRECTORY) {
		yaffs_trace(YAFFS_TRACE_ALWAYS,
			"tragedy: Trying to add an object to a non-directory%s"
			, "" );
		BUG();
	}
				 ...
				 const YCHAR *new_name, int force, int shadows)
{
	...
	if (new_dir->variant_type != YAFFS_OBJECT_TYPE_DIRECTORY) {
		yaffs_trace(YAFFS_TRACE_ALWAYS,
			"tragedy: yaffs_change_obj_name: new_dir is not a directory%s"
			, "" );
		BUG();
	}
...
static void yaffs_flush_file_cache(struct yaffs_obj *obj)
{
	...
	if (cache)
		...
		yaffs_trace(YAFFS_TRACE_ERROR,
			"yaffs tragedy: no space during cache write%s", "" );
}
...
void yaffs_update_dirty_dirs(struct yaffs_dev *dev)
{
	...
	yaffs_trace(YAFFS_TRACE_BACKGROUND, "Update dirty directories%s", "" );
				     ...
				     const YCHAR *name)
{
	...
	if (!directory) {
		yaffs_trace(YAFFS_TRACE_ALWAYS,
			"tragedy: yaffs_find_by_name: null pointer directory%s"
			, "" );
		BUG();
		return NULL;
	}
	if (directory->variant_type != YAFFS_OBJECT_TYPE_DIRECTORY) {
		yaffs_trace(YAFFS_TRACE_ALWAYS,
			"tragedy: yaffs_find_by_name: non-directory%s", "" );
		BUG();
	}
...
int yaffs_guts_ll_init(struct yaffs_dev *dev)
{
	...
	yaffs_trace(YAFFS_TRACE_TRACING, "yaffs: yaffs_ll_init()%s", "" );

	if (!dev) {
		yaffs_trace(YAFFS_TRACE_ALWAYS,
			"yaffs: Need a device%s"
			, "" );
		return YAFFS_FAIL;
	}
	...
	if (!yaffs_check_dev_fns(dev)) {
		/* Function missing */
		yaffs_trace(YAFFS_TRACE_ALWAYS,
			"device function(s) missing or wrong%s", "" );

		return YAFFS_FAIL;
	}

	if (yaffs_init_nand(dev) != YAFFS_OK) {
		yaffs_trace(YAFFS_TRACE_ALWAYS, "InitialiseNAND failed%s", "" );
		return YAFFS_FAIL;
	}
...
int yaffs_guts_initialise(struct yaffs_dev *dev)
{
	...
	if (dev->is_mounted) {
		yaffs_trace(YAFFS_TRACE_ALWAYS, "device already mounted%s", "" );
		return YAFFS_FAIL;
	}
	...
	if (dev->param.chunks_per_block < dev->chunk_grp_size) {
		 ...
		 */
		yaffs_trace(YAFFS_TRACE_ALWAYS, "chunk group too large%s", "" );
		...
		if (dev->param.is_yaffs2) {
			if (yaffs2_checkpt_restore(dev)) {
				...
				yaffs_trace(YAFFS_TRACE_CHECKPOINT |
					YAFFS_TRACE_MOUNT,
					"yaffs: restored from checkpoint%s"
					, "" );
			} else {
	...
	if (init_failed) {
		/* Clean up the mess */
		yaffs_trace(YAFFS_TRACE_TRACING,
		  "yaffs: yaffs_guts_initialise() aborted.%s", "" );
	...
	yaffs_trace(YAFFS_TRACE_TRACING,
	  "yaffs: yaffs_guts_initialise() done.%s", "" );
	return YAFFS_OK;
}
