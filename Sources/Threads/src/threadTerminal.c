...
typedef enum __TermMessage_t 
{
	UART_RCV_STRING     = 0,            /* хороший прием                                    */
        ...
        UART_SND_LOG_NEED   = 2,            /* нужно отослать текстовое сообщение от программы  */
} TermMessage_t;
...
{  
  ...
  {  return;  }
                                                                            /* отсылаем эхо если начали получать команду */
  ...
  CircBuffer_push_one_tender( &terminal_rcv_buff, (u8)(ch & 0xFF) ) ;        /* помещаем символ в кольцевой буфер */
  ++uart_rcvmsg_len;
...
void task_terminal (void *pdata)
{
    ...
    while( TRUE ){
        ...
        if( terminal_command( qCurMessage, &terminal_rcv_buff, &terminal_snd_buff, last_cmd)) {     /*  обрабатываем команду и отсылаем ответ */
            uart_terminal_start_tx(&uart0_desc) ;
        }
                                                                                                
        terminal_send_log( qCurMessage, &terminal_rcv_buff, &terminal_snd_buff, &uart0_desc, last_cmd ); /* отсылаем лог если нужно */
    }
}
