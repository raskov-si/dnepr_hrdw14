...
<<<<<<< HEAD
extern const u32 val_VPowerLimit ;
=======
extern u32 val_VPowerLimit ;
>>>>>>> fffeb94... РћР±РѕСЂРѕС‚С‹ РІРµРЅС‚РёР»СЏС‚РѕСЂРѕРІ СѓРїСЂР°РІР»СЏСЋС‚СЃСЏ
...
<<<<<<< HEAD
	...
	FPGA_INTERRUPT,			//!< прерывание от плис
...
<<<<<<< HEAD
extern const u32 val_VPowerLimitSource;
=======
...
<<<<<<< HEAD
	SPI_FPGA_PsStatusTypedef cur_psu_status ;
	SPI_FPGA_PsStatusTypedef prev_psu_status ;
=======
...
<<<<<<< HEAD
	Dnepr_Refresh_FPGA_Presents() ; // перечитываем презенты и 
	// сбрасываем прерывания презентов от ПЛИС
	SPI_FPGA_GetIrqSr();
	// дискретные выводы ПЛИС
	SPI_FPGA_GetPsStatus( &cur_psu_status );
	memcpy( &prev_psu_status, &cur_psu_status, sizeof(SPI_FPGA_PsStatusTypedef) );
=======
...
<<<<<<< HEAD
					Dnepr_Measure_SetPowerLed( (SPI_FPGA_LedTypedef){SPI_FPGA_YELLOW, FALSE} );
				// если у хотя бы одного БП не PowerGood -- мигаем желтым
				} else if( 	(!Dnepr_Backplane_GetPSU_Status()->tPs1.bPowerGood	) ||
							(!Dnepr_Backplane_GetPSU_Status()->tPs2.bPowerGood	) ){
					Dnepr_Measure_SetPowerLed( (SPI_FPGA_LedTypedef){SPI_FPGA_YELLOW, TRUE} );
				// всё хорошо
				} else {
					Dnepr_Measure_SetPowerLed( (SPI_FPGA_LedTypedef){SPI_FPGA_GREEN, FALSE} );
=======
...
<<<<<<< HEAD
        	case FPGA_INTERRUPT :
				...
				// сбрасываем флаг в ПЛИС до того как перечитаем параметры, чтобы не пропустить второе изменение презентов
				SPI_FPGA_GetIrqSr();
				OSTimeDly( 100 );
				// перечитываем презенты из ПЛИС
				Dnepr_Refresh_FPGA_Presents() ;
=======
...
<<<<<<< HEAD
static const task_message_t __fpgamess = { FPGA_INTERRUPT, 0 };
void Dnepr_DControl_FPGA_Interrupt()
{
	if( __qRcvQueue  ){
...
<<<<<<< HEAD
			...
			if( __slot_OnOff( i ) == PERMITTED_START ){
				...
				// устройство не включено и не выключено 
				} else {
...
<<<<<<< HEAD
			// если слот выключен -- запрещаем запуск hotswap'а
			if( __aPwrStates[ i ] == HSSLOT_REGULAR_OFF){
				__fSlotPower[ i ] = 0 ;
...
<<<<<<< HEAD
extern const u32 val_Slot1Enable ;
extern const u32 val_Slot2Enable ;
extern const u32 val_Slot3Enable ;
extern const u32 val_Slot4Enable ;
extern const u32 val_Slot5Enable ;
extern const u32 val_Slot6Enable ;
extern const u32 val_Slot7Enable ;
extern const u32 val_Slot8Enable ;
extern const u32 val_Slot9Enable ;
extern const u32 val_Slot10Enable ;
extern const u32 val_Slot11Enable ;
extern const u32 val_Slot12Enable ;
extern const u32 val_Slot13Enable ;
=======
const f32 __curMaxPower()
{
	size_t i ;
	f32 cur_max = 0 ;
